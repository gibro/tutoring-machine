...
class block_chatbot_content_extractor {
    ...
    private function extract_glossaries() {
        ...
        foreach ($this->modinfo->get_instances_of('glossary') as $glossary_instance) {
            ...
            if ($glossary) {
                // Start building glossary content
                ...
                if (!empty($h5p->json_content)) {
                    try {
                        ...
                        if (isset($content['questions'])) {
                            ...
                            // Try to extract useful text from the content
    ...
    /**
     * Extract text content from a PDF file with improved caching
     *
    ...
    private function extract_pdf_content($file) {
        ...
        try {
            ...
            // Method 1: Try Poppler's pdftotext (common on Linux servers)
            $pdftotext_paths = [
                '/usr/bin/pdftotext',
            ...
            if ($pdftotext_path !== null) {
                error_log("Using pdftotext at $pdftotext_path for PDF extraction");
                // Extract text using pdftotext
                $tempoutput = $tempdir . '/' . $content_hash . '.txt';
                $command = escapeshellcmd($pdftotext_path) . ' ' . 
                          escapeshellarg($tempfile) . ' ' . 
                          escapeshellarg($tempoutput);
                
                exec($command, $output, $return_var);
                
                if ($return_var === 0 && file_exists($tempoutput)) {
            ...
            // Method 2: Try using pdf2text PHP class if available
            if (empty($text) && class_exists('pdf2text')) {
                error_log("Using pdf2text PHP class for PDF extraction");
                try {
                    $pdf = new pdf2text();
                    $pdf->setFilename($tempfile);
                    $pdf->decodePDF();
                    $text = $pdf->output();
                    ...
                    error_log("Error using pdf2text: " . $e->getMessage());
                }
            }
            
            // Method 3: Try using the external service as a fallback
            if (empty($text)) {
                error_log("Trying external PHP solution for PDF extraction");
                // Include PDF parser library if available
                $pdf_parser_available = false;
                
                // Try to load PDF parser library
                if (file_exists($CFG->dirroot . '/blocks/chatbot/vendor/autoload.php')) {
                    include_once($CFG->dirroot . '/blocks/chatbot/vendor/autoload.php');
                    $pdf_parser_available = class_exists('\\Smalot\\PdfParser\\Parser');
                }
                
                if ($pdf_parser_available) {
                    try {
                        error_log("Using Smalot\\PdfParser for extraction");
                        ...
                        error_log("Error using PdfParser: " . $e->getMessage());
                    }
                }
            }
            
            // Method 4: Try using PHP built-in function imagick if available
            if (empty($text) && extension_loaded('imagick') && class_exists('Imagick')) {
                error_log("Using PHP Imagick extension for PDF extraction");
                try {
                    $im = new Imagick();
                    $im->readImage($tempfile . '[0]'); // Read first page
                    $im->setImageFormat('png');
                    
                    // Use OCR if available
                    if (function_exists('tesseract_ocr')) {
                        $tempimg = $tempdir . '/' . $content_hash . '.png';
                        file_put_contents($tempimg, $im->getImageBlob());
                        $text = tesseract_ocr($tempimg);
                        unlink($tempimg);
                    } else {
                        $text = "[Der PDF-Inhalt konnte nicht extrahiert werden. Es wurde nur ein Bild erkannt.]";
                    }
                } catch (Exception $e) {
                    error_log("Error using Imagick: " . $e->getMessage());
                }
            }
            
            // If all methods failed, provide an informative message
            if (empty($text)) {
                error_log("All PDF extraction methods failed");
                $text = "[PDF-Inhalt konnte nicht extrahiert werden. Um PDFs zu durchsuchen, installieren Sie bitte eine der folgenden Optionen auf dem Server:\n1. pdftotext (Poppler-Utils)\n2. Smalot/PdfParser PHP-Bibliothek\n3. PHP Imagick-Erweiterung mit OCR]";
            }
            ...
            // Save to cache with the new cache manager
            if (!empty($text)) {
                $metadata = [
    ...
    // The PDF caching functionality has been moved to the cache_manager class
            ...
            if ($forum) {
                ...
                foreach ($discussions as $discussion) {
                    ...
                    foreach ($posts as $post) {
                        // Skip if it's just a reply without substantial content
    ...
    private function extract_assignments() {
        ...
        foreach ($this->modinfo->get_instances_of('assign') as $assign_instance) {
            ...
            if ($assign) {
                // Start building assignment content
