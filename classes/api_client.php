...
abstract class block_chatbot_api_client_base implements block_chatbot_api_client_interface {
     ...
     * @param resource $verbose The file handle for verbose output
     */
     ...
     * @param float $request_time The time taken for the request
...
class block_chatbot_openai_client extends block_chatbot_api_client_base {
    ...
    /**
     * Extract the response text from API result
     * 
    ...
    protected function extract_response_text($result) {
        ...
        // Get and sanitize the response content
...
class block_chatbot_google_client extends block_chatbot_api_client_base {
    ...
    protected function prepare_request_data($messages) {
        ...
        // Convert OpenAI-compatible message format to Gemini format
     ...
     *
     * @param array $messages The array of messages in OpenAI format
     * @return array The messages in Gemini format
     */
    private function convert_to_gemini_format($messages) {
        ...
        foreach ($messages as $message) {
            if ($message['role'] === 'system') {
                ...
                // so we collect them to add as context
     ...
     *
     * @param array $gemini_messages The messages in Gemini format
     * @param string $system_content The collected system content
...
/**
 * Anthropic Claude API client implementation - REMOVED
 * This class has been removed since the integration with Claude API is not currently used
 */
...
class block_chatbot_api_client {
     ...
     * @return block_chatbot_api_client_interface The API client
     */
    public static function create($api_key, $model = null) {
        ...
        switch ($provider) {
            ...
            case 'anthropic':
                // Anthropic integration has been removed
                error_log('Anthropic integration is not supported');
    ...
    /**
     * Constructor - deprecated, use static create() method instead
     *
     * @param string $api_key The API key
     * @deprecated Since version 3.4.0
     */
    public function __construct($api_key) {
        debugging('Direct instantiation of block_chatbot_api_client is deprecated. Use block_chatbot_api_client::create() instead.', 
                  DEBUG_DEVELOPER);
                  
        // For backward compatibility, create an OpenAI client
        $this->client = self::create($api_key);
    }
    
    /**
     * Magic method to forward calls to the created client
     *
    ...
    public function __call($name, $arguments) {
        if (!isset($this->client)) {
            debugging('API client not properly initialized', DEBUG_DEVELOPER);
            return false;
        }
