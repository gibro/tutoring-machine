...
class block_chatbot_message_processor {
    ...
    public function truncate_response() {
        ...
        if (strlen($this->response_text) <= $primary_limit) {
            return $this;
        }
        ...
        // If no sentence endings found, return the full text
        if (empty($matches[0])) {
            return $this;
        }
        ...
        foreach ($matches[0] as $match) {
            ...
            // If the position is beyond our primary length but we don't have a primary end yet
            if ($position > $primary_limit && $last_sentence_end === null) {
                // Use the first sentence end we find after primary limit
                $last_sentence_end = $position;
            }
            
            // If the position is beyond our secondary length but we don't have a secondary end yet
            if ($position > $secondary_limit && $secondary_sentence_end === null) {
                // Use the first sentence end we find after secondary limit
        ...
        // If we found a primary limit sentence ending, use that
        if ($last_sentence_end !== null) {
            $this->response_text = substr($this->response_text, 0, $last_sentence_end);
            return $this;
        }
        
        // If no suitable sentence end was found, return the full text
        return $this;
    }
    
    /**
     * Get response as an assistant message object
     *
     * @return array Message object with role and content
     */
    ...
    /**
     * Get and validate conversation history from request
     * 
    ...
    /**
     * Get and validate course ID from request
     * 
    ...
    /**
     * Get and validate block instance ID from request
     * 
